...
def get_price_tezos():
  ...
  response = requests.get('https://min-api.cryptocompare.com/data/price?fsym=XTZ&tsyms=USD')
  response_json = response.json()
...
def get_price_tether():
  ...
  response = requests.get('https://min-api.cryptocompare.com/data/price?fsym=USDT&tsyms=USD')
  response_json = response.json()
...
def lalala(message):
    if message.chat.type == 'private':
        if message.text == 'Генератор паролей🔐':
            ...
            item6 = types.InlineKeyboardButton("Tether", callback_data='teth')
            item7 = types.InlineKeyboardButton("EOS", callback_data='eos')
            item8 = types.InlineKeyboardButton("Tezos", callback_data='xtz')
...
def callback_inline(call):
    try:
        if call.message:
            if call.data == 'btc':
                bot.send_message(call.message.chat.id, get_price_btc() + ' USD')
                sti = open('images/stonks-template.webp', 'rb')
                bot.send_sticker(call.message.chat.id, sti)
            elif call.data == 'eth':
                bot.send_message(call.message.chat.id, get_price_eth() + ' USD')
            elif call.data == 'xrp':
                bot.send_message(call.message.chat.id, get_price_xrp() + ' USD')
            elif call.data == 'bchd':
                bot.send_message(call.message.chat.id, get_price_bitcoincash() + ' USD')
            elif call.data == 'ltc':
                bot.send_message(call.message.chat.id, get_price_ltc() + ' USD')
            elif call.data == 'teth':
                bot.send_message(call.message.chat.id, get_price_tether() + ' USD')
            elif call.data == 'eos':
                bot.send_message(call.message.chat.id, get_price_eos() + ' USD')
            elif call.data == 'xtz':
                bot.send_message(call.message.chat.id, get_price_tezos() + ' USD')
            elif call.data == 'xmr':
                bot.send_message(call.message.chat.id, get_price_monero() + ' USD')  
